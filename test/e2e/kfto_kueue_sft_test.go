...
func TestPytorchjobWithSFTtrainer(t *testing.T) {
	...
	tuningjob_cr, err := test.Client().Kubeflow().KubeflowV1().PyTorchJobs(namespace.Name).Create(test.Ctx(), tuningjob, metav1.CreateOptions{})
	test.Expect(err).NotTo(HaveOccurred())
	test.T().Logf("Created PytorchJob %s/%s successfully", tuningjob_cr.Namespace, tuningjob_cr.Name)
	test.Eventually(PytorchJob(test, namespace.Name, tuningjob_cr.Name), TestTimeoutLong).Should(WithTransform(PytorchJobCondition, Equal("PyTorchJobSucceeded")))
	test.T().Logf("PytorchJob %s/%s ran successfully", tuningjob_cr.Namespace, tuningjob_cr.Name)
}
